...
- **多格式互转**：任意加载、任意保存；跨格式写入保留目标文件其余配置
...
- **延迟 / 连通性测试**：Providers 标题行一键测试全部厂商连通性；每个 provider 可并发测试全部模型延迟（超时 8 秒），结果显示在厂商与模型卡片上，失败显示错误码
- **配置预览 / 编辑**：右侧面板实时显示待保存内容，JSON / YAML 语法高亮，可直接编辑（改动实时应用到表单并自动保存），支持 Ctrl+F 查找、拖动分隔条调整宽度
